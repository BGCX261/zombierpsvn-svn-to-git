...
if ( CLIENT ) then
	...
	killicon.AddFont("cse_smokegrenade","CSKillIcons",SWEP.IconLetter,Color(255,80,0,255))
end
...
function SWEP:Think()
	if CLIENT then
		if not self.colorstr then
			self.colorstr = self.Owner:GetInfo("cl_crosshair_color")
			if self.colorstr then
				self.color = self.string_explode(" ", self.colorstr)
				self.color = Color(self.color[1], self.color[2], self.color[3])
			else
				self.color = Color(240,240,240)
				self.colorstr = "240 240 240"
				CreateClientConVar("cl_smokegrenade_color","240 240 240",true,true)
			end
		elseif self.Owner:GetInfo("cl_smokegrenade_color") ~= self.colorstr then
			self.colorstr = self.Owner:GetInfo("cl_smokegrenade_color")
			self.color = string.Explode(" ", self.colorstr)
			self.color = Color(self.color[1], self.color[2], self.color[3])
		end
	end
	if self.Next < CurTime() then
		...
		elseif self.Primed == 2 then
			...
			if SERVER then
				local ent = ents.Create("cse_ent_smokegrenade")
				
				ent:SetPos(self.Owner:GetShootPos())
				ent:SetAngles(Vector(1,0,0))
				ent:SetKeyValue("smokecolor",self.Owner:GetInfo("cl_smokegrenade_color") or "255 255 255")
				ent:Spawn()
